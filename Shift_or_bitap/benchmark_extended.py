"""
Benchmarking and Testing Harness for Shift-Or Extended Algorithm

This module provides testing and benchmarking capabilities for the extended
Shift-Or algorithm for patterns > 64 bp (exact matching only).
...
from shift_or_extended import ShiftOrExtended, search_multiple_patterns
from shift_or_utils import read_fasta_file, read_fasta_single_sequence

class ShiftOrExtendedBenchmark:
    """
    Benchmarking suite for Shift-Or extended algorithm (>64 bp patterns).
    """
    ...
    def benchmark_scaling(self, text: str, pattern_length: int = 100,
        ...
        Args:
            ...
            pattern_length: Length of pattern to extract (>64 bp)
    ...
    def benchmark_pattern_lengths(self, text: str,
                                  ...
                                  trials: int = 10):
        """
        Benchmark impact of varying pattern lengths (>64 bp).
        ...
        if pattern_lengths is None:
            pattern_lengths = [100, 200, 400, 800]
        ...
        for length in pattern_lengths:
            ...
            results.append({
                ...
                'implementation': 'multi-word'
            })

        return results

    def benchmark_multiple_patterns(self, text: str, k: int = 50,
                                   base_length: int = 100, step: int = 10):
        """
        Benchmark searching for multiple extended patterns.

        Args:
            text: Full DNA sequence
            k: Number of patterns to test
            base_length: Starting pattern length (>64 bp)
        ...
        patterns = []
        for i in range(k):
            length = base_length + step * i  # 100, 110, 120, ..., 590
            start = i * 200
    ...
    def plot_scaling(self, results, output_path, dataset_name):
        ...
        ax1.set_title(f'Shift-Or Extended (>64 bp): Time vs Slice Size (n) - {dataset_name}')
    ...
    def plot_pattern_length(self, results, output_path, dataset_name):
        """Generate pattern length plots showing multi-word performance."""
        if not results:
            return
        ...
        ax1.set_title(f'Shift-Or Extended: Multi-word Performance (>64 bp) - {dataset_name}')
        ax1.grid(True, alpha=0.3)
        ax1.legend()
        ax1.text(0.98, 0.95, 'Multi-word implementation', 
...
def run_benchmark_on_dataset(fasta_file, output_dir):
    """
    Run complete benchmark suite on a single dataset for extended patterns.
    ...
    benchmark = ShiftOrExtendedBenchmark()

    # 1. Scaling benchmark (using 100 bp pattern)
    print("  Running scaling benchmark (100 bp pattern)...")
    scaling_results = benchmark.benchmark_scaling(text, pattern_length=100, num_runs=3)
    ...
    print("  Running pattern length benchmark (>64 bp)...")
...
if __name__ == "__main__":
    if len(sys.argv) > 2:
        ...
        print("Usage: python benchmark_extended.py <fasta_file> <output_dir>")
