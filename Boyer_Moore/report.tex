...
Let $j$ be the position where mismatch occurs. The shift is:
\begin{equation}
s = \max\{\delta_1(T[i+j], j), \delta_2(j)\}
\end{equation}

By construction:
\begin{itemize}
    \item $\delta_1$ shifts to align with the rightmost occurrence of the mismatched character
    \item $\delta_2$ shifts to align with another occurrence of the matched suffix
\end{itemize}

If there is a match at $k$ where $i < k < i + s$, then either:
\begin{itemize}
    \item The character at $T[i+j]$ must appear at position $k + j - i$ in $P$, contradicting the definition of $\delta_1$
    \item The suffix matched at $i$ must also occur at $k$, contradicting the definition of $\delta_2$
\end{itemize}

Therefore, no match can exist at $i < k < i + s$. \textbf{Contradiction.}
...
\subsection{Correctness of Heuristics}

\subsubsection{Bad Character Correctness}

If $T[i+j] = c$ and $c$ does not appear in $P[0..j-1]$, then no match can exist with $c$ aligned to any position in $P[0..j-1]$. Safe to shift past $c$.

\subsubsection{Good Suffix Correctness}

If suffix $P[j+1..m-1]$ matched in $T$, but $P[j] \neq T[i+j]$:
\begin{itemize}
    \item Any match must have the same suffix
    \item The suffix must occur elsewhere in $P$ with a different preceding character
    \item Or a prefix of $P$ must match the suffix
\end{itemize}

The good suffix table ensures we shift to the nearest such position.
...
\textbf{Good Suffix Table}: $\delta_2 = [6, 6, 6, 6, 2, 1]$
...
Good suffix shift: $\delta_2[4] = 2$

\textbf{Shift} = $\max(2, 2) = 2$

\textbf{Alignment 2} ($i = 2$):
\begin{verbatim}
Text:    A C G T A C G G A T G C G A A T T C A G T A C G
Pattern:     G A A T T C
             -----------
...
Compare: $P[5] = $ ``C'' vs $T[7] = $ ``G'' \textbf{X MISMATCH}

Bad char shift: ``G'' at position 0, shift = $5 - 0 = 5$

Good suffix shift: $\delta_2[5] = 1$

\textbf{Shift} = $\max(5, 1) = 5$

\textbf{Alignment 3} ($i = 7$):
\begin{verbatim}
Text:    A C G T A C G G A T G C G A A T T C A G T A C G
Pattern:             G A A T T C
                     -----------
...
Compare: $P[5] = $ ``C'' vs $T[12] = $ ``G'' \textbf{X MISMATCH}

Shift = 5 (similar to above)

\textbf{Alignment 4} ($i = 12$):
\begin{verbatim}
Text:    A C G T A C G G A T G C G A A T T C A G T A C G
Pattern:                         G A A T T C
                                 -----------
...
\begin{enumerate}
    ...
    \item \textbf{Large shifts} (2, 5, 5 positions) skipped impossible alignments
    \item \textbf{Only 4 alignments} checked instead of 19 (naive approach)
    \item \textbf{Total comparisons}: $\sim$10 instead of $\sim$114 (naive)
\end{enumerate}
...
\subsection{Shift Visualization}

\begin{figure}[!htb]
\centering
\begin{tikzpicture}[scale=1.0]
    % Text sequence
    \node at (-2, 0) {\textbf{Text:}};
    \foreach \i/\c in {0/A,1/C,2/G,3/T,4/A,5/C,6/G,7/G,8/A,9/T,10/G,11/C,12/G,13/A,14/A,15/T,16/T,17/C} {
        \node at (\i*0.5, 0) {\small \c};
        \draw (\i*0.5-0.2, -0.25) rectangle (\i*0.5+0.2, 0.25);
    }
    
    % Pattern alignment 1
    \node at (-2, -1.5) {\textbf{Align 1:}};
    \foreach \i/\c in {0/G,1/A,2/A,3/T,4/T,5/C} {
        \node at (\i*0.5, -1.5) {\small \c};
        \draw[red, very thick] (\i*0.5-0.2, -1.75) rectangle (\i*0.5+0.2, -1.25);
    }
    \draw[->, very thick, red] (3*0.5, -2.0) -- node[right, xshift=0.2cm] {\small Shift by 2} (3*0.5, -2.5);
    
    % Pattern alignment 2
    \node at (-2, -3.5) {\textbf{Align 2:}};
    \foreach \i/\c in {0/G,1/A,2/A,3/T,4/T,5/C} {
        \node at ((\i+2)*0.5, -3.5) {\small \c};
        \draw[blue, very thick] ((\i+2)*0.5-0.2, -3.75) rectangle ((\i+2)*0.5+0.2, -3.25);
    }
    \draw[->, very thick, blue] (5*0.5, -4.0) -- node[right, xshift=0.2cm] {\small Shift by 5} (5*0.5, -4.5);
    
    % Pattern alignment 3 (skipped alignments)
    \node at (-2, -5.5) {\textbf{Align 3:}};
    \foreach \i/\c in {0/G,1/A,2/A,3/T,4/T,5/C} {
        \node at ((\i+7)*0.5, -5.5) {\small \c};
        \draw[orange, very thick] ((\i+7)*0.5-0.2, -5.75) rectangle ((\i+7)*0.5+0.2, -5.25);
    }
    \draw[->, very thick, orange] (8.5*0.5, -6.0) -- node[right, xshift=0.2cm] {\small Shift by 5} (8.5*0.5, -6.5);
    
    % Pattern alignment 4 (match found)
    \node at (-2, -7.5) {\textbf{Align 4:}};
    \foreach \i/\c in {0/G,1/A,2/A,3/T,4/T,5/C} {
        \node at ((\i+12)*0.5, -7.5) {\small \c};
        \draw[green!60!black, very thick] ((\i+12)*0.5-0.2, -7.75) rectangle ((\i+12)*0.5+0.2, -7.25);
    }
    \node at (15*0.5, -8.3) {\color{green!60!black} \Large \textbf{MATCH!}};
\end{tikzpicture}
\caption{Visualization of pattern shifts during search}
\label{fig:shifts}
\end{figure}
